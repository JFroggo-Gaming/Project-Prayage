...
public class CraftingPanelController : MonoBehaviour
{
    ...
    public Sprite defaultSlotSprite; // Dodaj pole na domyślny sprite slotu
    ...
    private void UpdateGridLayout()
    {
        ...
        {
            GameObject newSlotObject = Instantiate(slotPrefab, gridLayoutGroup.transform);
            UIInventoryCraftingSlot newCraftingSlot = newSlotObject.GetComponent<UIInventoryCraftingSlot>();

            // Przypisz domyślny sprite dla nowo utworzonego slotu
            newCraftingSlot.SetDefaultSprite();

            // Dodaj obsługę Drag & Drop dla slotów rzemieślniczych
            AddDragAndDrop(newCraftingSlot);
        }
